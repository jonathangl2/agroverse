...
import type { ZoneId, SkinId } from "@/types";
...
export default function GameApp() {
  ...
  // ── Onboarding — se muestra siempre al inicio ──────────────────────────
  if (!onboarded) {
  ...
  if (!wallet.isConnected) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-8 text-center"
        ...
        <p className="text-green-600 text-sm mb-1">Colombia · Season 1</p>
        <p className="text-green-700/80 text-sm mb-8 max-w-xs">
          Cultiva, cosecha y compite con agricultores de todo el país.
          Gana puntos convertibles en USDT.
        </p>
  ...
  // ── Elegir zona ────────────────────────────────────────────────────────────
  if (!zone) return <ZoneSelector onSelect={setZone} />;

  // ── Juego ──────────────────────────────────────────────────────────────────
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "#fef9f0" }}>

      {/* ── Barra superior fija (reemplaza WalletBar) ── */}
      <div className="fixed top-0 left-0 right-0 z-40 shadow-md">
        <div className="flex items-center justify-between bg-green-700 px-5 py-3">
          <div className="flex items-center gap-2">
            <img src="/assets/logo.png" alt="AgroVerse" className="w-8 h-8 rounded-lg" />
            <span className="font-bold text-white text-base">AgroVerse</span>
          </div>
          <div className="flex items-center gap-2">
            {/* Puntos */}
            <div className="flex items-center gap-1 bg-green-800/60 px-3 py-1.5 rounded-full">
              <span className="text-yellow-300 font-bold text-sm font-mono">⭐ {points.toLocaleString()}</span>
              <span className="text-green-300 text-xs">pts</span>
            </div>
            {/* Wallet */}
            {wallet.isConnected && wallet.address ? (
              <span className="bg-white/20 text-white text-xs font-mono px-2 py-1.5 rounded-full">
              ...
              <button
                onClick={wallet.connect}
                disabled={wallet.isLoading}
                className="bg-white text-green-700 px-3 py-1.5 rounded-full text-xs font-bold transition disabled:opacity-50"
              >
                {wallet.isLoading ? "..." : "Conectar"}
              </button>
            ) : null}
          </div>
        </div>
        ...
        {demoMode && (
          <div className="bg-amber-100 border-b border-amber-300 text-amber-700 text-xs text-center py-1.5 px-4">
            ...
            <button onClick={() => { setDemoMode(false); setZone(null); }} className="underline font-semibold">
              Salir
            </button>
          </div>
        )}
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-24 space-y-4">
        {tab === "farm"    && <FarmView zoneId={zone} onPointsEarned={handlePointsEarned} />}
        {tab === "profile" && (
          <Profile
        ...
        {tab === "ranking" && <Ranking currentAddress={wallet.address} currentPoints={points} />}
